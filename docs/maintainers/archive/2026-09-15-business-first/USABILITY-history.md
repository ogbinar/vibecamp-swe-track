...
The detailed remediation sequence is in [PLAN.md](PLAN-history.md). Re-score every
...
Scope: local source and executable maintenance review of Catalog M0/M1, POS M2,
Ecommerce M6, and Social M9. Python 3.13.11 and exact Air 0.48.1 were used. The
review followed each controller without opening its reference first, then ran
the page/API/form/HTMX/SSE and project gates recorded in the
[BF provenance manifest](../../../../docs/maintainers/archive/2026-09-15-business-first/README.md#pilot-validation).

The actor, harm, product objective, first command, expected result, recovery,
proof, and next action were identifiable from each controller. Consolidated
references preserved their original anchors. Wrong turns found during the pass
were stale `#literal-command-map` and `#evidence` links; both were repaired and
the link validator rerun. Air pages remained out of OpenAPI, no browser route
called its own API, the Catalog error form preserved safe values, M6 used one
focused HTMX fragment, and M9 used one non-durable Air SSE transport while the
domain correctness work stayed learner-owned.

Maintenance score vector: audience/outcome 4, first run 4, executable
...
below 3. This is a local maintainer assessment, not a named-human or hosted
result.

Decision: **ROLL OUT**. The simpler business-first/four-file/Air composition
pattern preserved the local gates and reduced first-route competition. External
sandbox, hosted GitHub, deployment, and named-human evidence remain pending.
