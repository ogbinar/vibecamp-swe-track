...
Status: **BUSINESS-FIRST + AIR-PRIMARY CONTRACT RECORDED — APPLICATION CODE NOT STARTED — EXTERNAL EVIDENCE PENDING**

This is the active completion ledger for [PLAN.md](PLAN-history.md). Checked items were
...
latest local repair below is complete. The active BF0–BF6 phase now includes
the Air-primary/FastAPI-backend migration. No dependency, lockfile, runtime, or
application code has been changed yet.
Pre-existing unchecked items still require provider authority, a human learner,
GitHub-hosted observation, or a learner-authorized real target. Partial
substitutes are recorded honestly and do not count as stronger external evidence.
...
- [x] **Cold-test repository → M0 orientation.** Target: local source-rendered
...
- [x] **Record the baseline learner-tree observation.** Target: local source-rendered
...
IA0/CU0 local evidence: [execution baseline](PLAN-history.md#2026-09-13-ia0-and-cu0-execution-baseline). GitHub-rendered and human observations remain external; local source walkthrough is the explicitly bounded substitute.


Local closeout evidence: [final transitions and tests](USABILITY-history.md#ia5-and-cu5-final-transition-review),
[dispositions](PLAN-history.md#cu1-disposition-and-authority-decisions), and
...
Canonical rationale and frozen decisions: [active CS plan](PLAN-history.md#active-plan--career-shifterself-paced-instructional-revision).
...
Local closeout evidence (2026-09-14): [exact matrix and limitations](PLAN-history.md#cs6-local-closeout--2026-09-14)
and [bounded maintenance review](USABILITY-history.md#2026-09-14-cs6-local-closeout-review).
...
[PG4 independent fresh-context review](USABILITY-history.md#2026-09-14-pg4-independent-fresh-context-review).
...
## LOCAL RESUME REPAIRS — authority and evidence scope

- [x] **LR1 Identify the highest-impact local contradiction.** Compare the
  canonical M4 roadmap row with the frozen CS0.3 decision, M4 controller,
  challenge, acceptance gate, and fixed project brief. Confirmed that only
  `CURRICULUM.md` still advertised returns/promotions instead of the optional
  cashier display name.
- [x] **LR2 Repair and protect the canonical wording.** Update the M4 roadmap
  cell to the optional cashier display name, require that stable marker in the
  curriculum validator, and add a controlled mutation that restores the stale
  wording and must fail for the intended reason.
- [x] **LR3 Validate and audit the repair.** Run the curriculum validator,
  controlled mutation suite, focused M4 terminology scan, `git diff --check`,
  and changed-file audit. Complete only if M0–M10, 11×7, 58 traces, and all
  external/provider/human checkboxes remain unchanged.
- [x] **LR4 Audit checked evidence-scope claims independently.** Confirmed that
  two checked IA/CU targets said `GitHub-rendered` while their dated evidence
  records only local source traversal and the hosted observation remains
  unchecked.
- [x] **LR5 Repair and protect the local target labels.** Relabel only those two
  checked targets as local source-rendered, clarify the corresponding plan
  wording, and add a controlled validator mutation for the settled local label.
- [x] **LR6 Re-run the local closeout gate.** Require validator, mutations,
  focused evidence-scope scan, `git diff --check`, and changed-file audit to
  pass without checking the hosted-rendering task.

LR1–LR6 are locally complete. The repairs change no learner implementation or
gate. All external/provider/hosted/deployment and named-human work above remains
unchecked and unauthorized.

## ACTIVE LOCAL PHASE — business-first Air-primary learner path

Canonical rationale and target contract:
[PLAN.md](PLAN-history.md#active-plan--business-first-air-primary-learner-path).

Execution order is BF0 → BF1 → BF2 decision → BF3 → BF4 → BF5 → BF6. Do not
rewrite the remaining milestones before the Catalog M0–M1 plus M2/M6/M9 pilot
returns `ROLL OUT`.
Check an item only after its stated observation and validation pass. Earlier
checked work remains valid historical evidence, but earlier presentation freezes
do not override this replacement plan.

### BF0 — Freeze the replacement contract and Air boundary

- [x] **BF0.1 Record the business-first career-shifter diagnosis.** Target:
  `docs/maintainers/business-first-career-shifter-review.md` and maintainer
  navigation. Expected: exact remove/merge/rename/move/rewrite candidates,
  Pareto priorities, and evidence limits; no lesson rewrite. Validate: source
  citations resolve and the review states that it is local/read-only.
- [x] **BF0.2 Authorize reopening prior presentation decisions.** Target:
  `PLAN.md`. Expected: the six-column root map, seven-file milestone contract,
  fixed controller order, up-front terms, repeated process cues, and prominent
  completed chronology are explicitly superseded; protected engineering and
  evidence constraints remain named. Validate: decision review against the
  business-first objective and existing learner-solution boundary.
- [x] **BF0.3 Publish the staged implementation and acceptance plan.** Target:
  `PLAN.md` and this active checklist. Expected: four-file target, controller
  grammar, Air-primary/FastAPI-backend architecture, Python 3.13 and exact Air
  pin target, outcome-first titles, pilot/rollout/cleanup sequence, rollback
  boundaries, and external-work separation are explicit. Validate:
  `git diff --check` and manual unchecked-item audit. No milestone content is
  changed by BF0.
- [x] **BF0.4 Replace the repository working and stack contracts.** Depends on:
  BF0.1–BF0.3. Targets: `AGENTS.md`, `STACK.md`, `CURRICULUM.md`, and active
  `PLAN.md`/`TODO.md`. Expected: exactly one active four-file/business-first
  contract; Air owns pages/forms/components/fragments; FastAPI retains
  JSON/OpenAPI and operational endpoints; both share Python behavior in one
  process. Python 3.13 and Air `0.48.1` are migration targets, while the current
  Python 3.12 code remains honestly recorded. Include a bounded mixed-tree rule
  and earned forms→HTMX→SSE order. Validate: contract/contradiction scan,
  curriculum validator, mutation suite, and `git diff --check`; rollback:
  restore only the canonical contract edits without touching learner code.
- [x] **BF0.5 Prepare transitional validator behavior.** Depends on: BF0.4.
  Targets: `scripts/check_curriculum.py` and
  `scripts/test_validator_mutations.py`. Expected: the validator reports which
  milestones use the old or new file contract, rejects hybrids missing unique
  content, cannot falsely report global four-file completion early, and records
  current Python/Air migration state without requiring Air before BF1. Validate:
  controlled old/new/hybrid and pre/post-Air cases plus the full mutation suite;
  rollback: remove the transition rules and mutations together.

### BF1 — Catalog M0–M1 Air and learner-path pilot

- [x] **BF1.1 Prepare transitional validation and inventory Catalog content.**
  Depends on: BF0.5. Targets: all M0/M1 milestone files, Catalog application,
  contracts, tests, Python/runtime declarations, and inbound links. Expected:
  every unique learner item has a merge destination; every Python 3.12 reference
  has a classified migration disposition; existing FastAPI routes and OpenAPI
  assertions are listed before changes. Validate link/reference and route/test
  inventories; rollback: retain the old files and Python environment.
- [x] **BF1.2 Run the Python 3.13 and Air preflight on Catalog only.** Targets:
  Catalog `.python-version`, `pyproject.toml`, mypy setting, and lockfile.
  Expected: Python 3.13; exactly `air==0.48.1`; compatible explicit FastAPI
  backend/runtime dependencies; no other project changes yet. Validate locked
  sync, import/startup, Ruff, format, mypy, pytest, and dependency inspection.
  Rollback: restore the Catalog runtime files if the reviewed Air release cannot
  meet the current contracts cleanly.
- [x] **BF1.3 Compose Air over the existing FastAPI Catalog app.** Expected:
  `air.Air` is the exported ASGI app; an OpenAPI-excluded `AirRouter` owns `/`
  and `/app/...`; `/health`, `/products/sample`, M1 JSON routes, error shapes,
  response models, and OpenAPI remain compatible. Air and FastAPI handlers reuse
  shared Python behavior and never call each other over HTTP. Validate HTML/API
  content types, API regression, OpenAPI, app-factory isolation, and startup.
  Rollback: remove the Air composition while retaining unchanged backend code.
- [x] **BF1.4 Add the minimum Air product path.** Expected: M0 renders one useful
  catalog page with Air Tags; M1 adds one ordinary Pydantic-backed form using
  explicit `AirForm.from_request()` with safe value-preserving errors. Do not
  add HTMX, Jinja, AirDB, AirDragon/Tailwind, custom JavaScript, Node, or a
  separate frontend. Validate page/form tests and no duplicated business logic.
- [x] **BF1.5 Migrate M0/M1 to the four-file business-first contract.** Preserve
  every C/A ID, command, hint, evidence path, recovery route, and API lesson.
  Teach the product need before Air, and teach the backend/API behavior before
  framework detail. Validate content provenance, anchors, commands, acceptance,
  curriculum validator, mutations, and `git diff --check`.
- [x] **BF1.6 Run a fresh-context Catalog traversal and repair.** Test root→M0,
  M0 page, M0→M1, form failure, API proof, recovery, resume, and next action.
  Record and repair wrong turns; this local evidence cannot satisfy named-human
  or hosted review.

### BF2 — M2/M6/M9 stress pilots and rollout decision

- [x] **BF2.1 Migrate M2 and the POS shell.** Make stock/cart persistence visible
  through a thin Air surface without duplicating SQLAlchemy, Alembic, session,
  or transaction logic. Consolidate M2 to four files only after content and
  inbound-link inventories. Validate existing backend checks plus HTML/form,
  restart, link, provenance, and no-solution regression; rollback the M2 slice.
- [x] **BF2.2 Migrate M6 and earn the first HTMX interaction.** Preserve the
  provider fake, timeout/retry/webhook/reconciliation authority, local Core, and
  unchecked sandbox evidence. Introduce one Air fragment through `hx_*` only
  because uncertain payment status makes a full-page refresh inadequate. Keep a
  normal backend path testable. Validate fragment targeting, error/unknown state,
  API regression, and absence of custom JavaScript/business-logic duplication.
- [x] **BF2.3 Migrate M9 and earn Air SSE.** Measure the feed before Redis or
  realtime changes. Reuse the shared feed behavior, then use Air fragments and
  `SSEResponse` with the HTMX SSE extension only after the one-way latency need
  is proven. Preserve learner-built SSE, query-budget, cache authority,
  reconnect/gap/loss, and WebSocket-Stretch boundaries. Validate performance,
  fragment/SSE contracts, API regression, content provenance, and four-file
  structure.
- [x] **BF2.4 Record the rollout decision.** Depends on: all BF1/BF2 pilots.
  Target: dated PLAN/USABILITY evidence. Compare learner orientation, Air/FastAPI
  responsibility clarity, Python/setup cost, navigation, content preservation,
  OpenAPI/API compatibility, form/HTMX/SSE behavior, command executability, and
  gate parity. Record exactly `ROLL OUT`, `REVISE`, or `ROLL BACK`; do not
  continue on an ambiguous result.

### BF3 — Remaining milestone rollout

- [ ] **BF3.1 Migrate M3 and M4.** Reuse the POS Air shell and shared checkout
  behavior; keep transaction/race proof backend-driven and preserve the fixed M4
  `cashier_name` change. Add a service boundary only if page/API duplication or
  an invariant earns it. Validate each milestone independently.
- [ ] **BF3.2 Migrate M5 and the ecommerce shell.** Add the minimum Air login and
  order pages over the first-party secure cookie session. Preserve registration,
  authentication, authorization, ownership, state, negative-test, and JWT-
  Stretch contracts; do not add an Air auth product or second identity path.
- [ ] **BF3.3 Migrate M7 and M8.** Show accepted job and booking state through
  focused Air pages/fragments while keeping crash/replay and final-seat race
  evidence in deterministic backend/database harnesses. Preserve M7 durability
  and M8 concurrency semantics.
- [ ] **BF3.4 Migrate M10 and the production-shaped Air/FastAPI boundary.** Add a
  tenant-aware operator path without changing tenant, audit, readiness, release,
  restore, or recovery authority. Give local Core and optional real-target work
  separate done conditions; do not add a separate frontend image/service.
- [ ] **BF3.5 Audit all eleven controllers and five applications as one system.**
  Expected: common business-first grammar, outcome-first titles, one canonical
  command location, one next action, Air-primary UI, retained FastAPI APIs, and
  shared behavior without internal HTTP calls. Validate cross-milestone and
  cross-product consistency and repair only observed defects.

### BF4 — Root entrance and references

- [ ] **BF4.1 Replace the root six-column roadmap.** Target: `README.md`.
  Expected columns: `Milestone | Business problem | Product capability | Start
  here`; no concept, package, or provider inventory competes with the learner's
  product route. Preserve stable M0–M10 labels and correct links.
- [ ] **BF4.2 Move detail to its canonical owner.** Targets: `CURRICULUM.md`,
  `STACK.md`, `QUALITY-GATES.md`, `PROGRESS.md`, and milestone references.
  Expected: concept/tool/gate/progress facts and Air/FastAPI responsibility/timing
  remain available but are not duplicated in the root route. Validate authority
  and relative-link audit.
- [ ] **BF4.3 Verify the complete first-run path.** Traverse root → M0 → first
  block → proof → progress → M1 using a clean local source view. Record and
  repair wrong turns. Hosted GitHub rendering and named-human review remain
  separate unchecked evidence.

### BF5 — Safe cleanup and maintainer consolidation

- [ ] **BF5.1 Remove superseded milestone files in reviewed batches.** Depends
  on: every milestone content/link inventory passing. Remove `CONCEPTS.md`,
  `TOOLS.md`, `RESOURCES.md`, and `REVIEW.md` only after their content has moved
  to `REFERENCE.md`/`ACCEPTANCE.md` and all inbound links are updated. Record a
  per-milestone provenance and rollback manifest; never use broad destructive
  commands.
- [ ] **BF5.2 Consolidate completed maintenance chronology.** Move superseded
  execution history into a dated `docs/maintainers/archive/` bundle and leave
  concise canonical front doors for current PLAN, TODO, USABILITY, and analysis.
  Keep active external evidence items visible and unchecked. Validate links,
  provenance, history retention, and rollback paths before removing duplication.
- [ ] **BF5.3 Audit learner-visible file choice.** Expected: root README is the
  only entrance; a milestone presents four predictable choices; maintainer
  material does not appear as required study. Repair remaining parallel routes
  or misleading labels.

### BF6 — Validation, audit, repair, and local closeout

- [ ] **BF6.1 Enforce stable replacement facts.** Migrate validator and mutation
  coverage from 11×7 to 11×4, protect stable IDs/links/business summaries/tool
  classifications, Python 3.13 consistency, exact Air pin, Air/FastAPI route
  ownership, OpenAPI exclusion, and one-process/no-SPA boundaries. Avoid
  automated prose-quality claims. Every new rule needs a controlled failure.
- [ ] **BF6.2 Run the proportional local matrix.** Required: curriculum validator,
  mutation suite, relative links/anchors, secret/unsafe-fixture scan,
  `git diff --check`, affected starter lint/type/tests/migrations, deliberate
  failure/reset checks, HTML/form/HTMX/SSE and unchanged API/OpenAPI checks,
  container/runtime checks, and exact removed-content/provenance audit.
- [ ] **BF6.3 Perform an independent local simplicity audit and repair loop.**
  Test business-first clarity, plain language, cognitive load, jargon,
  navigation, obvious next steps, product usefulness, Air/FastAPI responsibility
  clarity, tool timing, recovery, and no-solution integrity. Repair and rerun
  until locally clean; do not call this named-human evidence.
- [ ] **BF6.4 Close the local phase truthfully.** Update PLAN, TODO, USABILITY,
  maintainer navigation, and status language with exact validation results and
  limits. Complete only when all locally required BF items are checked and no
  stronger external claim is implied.

### External evidence — unchanged and still unchecked

The earlier M6 sandbox, optional provider/tool endorsements, hosted GitHub
rendering/Actions observation, optional real deployment, and named-human
career-shifter reviews remain governed by their existing unchecked items. BF0
does not authorize or satisfy any of them.
