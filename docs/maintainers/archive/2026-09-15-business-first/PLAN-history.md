...
Status: **BUSINESS-FIRST + AIR-PRIMARY MIGRATION PLANNED — APPLICATION CODE NOT STARTED — EXTERNAL EVIDENCE PENDING**
Date: 2026-09-15

Latest plan: [business-first Air-primary learner path](#active-plan--business-first-air-primary-learner-path).
Current local implementation remains the completed
[Pareto post-closeout gap repair](#pareto-post-closeout-gap-repair).
...
[remaining external work](TODO-history.md#external-release-evidence--still-required).
...
- [the eight-persona repository review](PERSONA-REVIEW-history.md);
- [the conflict and resolution analysis](ANALYSIS-history.md);
- [the learner-experience rubric](USABILITY-history.md).
...
real-target endorsement remain external evidence in [TODO.md](TODO-history.md).
...
## Historical decisions from the completed remediation

These decisions describe the completed pre-BF candidate. The latest active plan
below supersedes the seven-file and FastAPI-only presentation decisions while
preserving the engineering, evidence, and learner-solution boundaries.
...
clean local source-rendered path. Hosted GitHub rendering remains a separate
external observation. Freeze the grammar above before broad edits.
...
IA2 decision: **ROLL OUT** local grammar; [pilot evidence](USABILITY-history.md#2026-09-13-ia-pilot-and-rollout-evidence). CU1 decisions above are now frozen for the authorized local batches. Named human and hosted observations remain external.
...
transition observations](USABILITY-history.md#ia5-and-cu5-final-transition-review)
...
## Local authority and evidence-scope repairs — 2026-09-14

A resume audit found one remaining locally actionable contradiction:
`CURRICULUM.md` still described the M4 product change as returns/promotions,
while the frozen decision, milestone controller, challenge, acceptance gate,
and project brief all require the single optional `cashier_name` receipt field.
That stale roadmap cell could send a learner toward an unsupported second Core
variant even though CS1.4 was recorded complete.

The roadmap now names the optional cashier display name. The stable CS validator
also requires that canonical M4 wording, and a controlled mutation proves that
reintroducing the stale returns/promotions wording is rejected. This is a narrow
authority repair: it changes no M4 behavior, gate, learner solution boundary,
milestone structure, provider status, or external evidence claim.

An independent read-only resume audit then found two checked IA/CU items whose
target labels still said `GitHub-rendered`, despite their recorded evidence being
an explicitly local source traversal. Their targets now say local source-rendered
Markdown; the genuine hosted-rendering observation remains external and
unchecked. The validator protects those two settled local-evidence labels with a
controlled mutation, without blocking a future separately evidenced hosted task.

## ACTIVE PLAN — Business-first Air-primary learner path

Status: **AIR-PRIMARY CONTRACT RECORDED — APPLICATION MIGRATION NOT STARTED**

Decision date: 2026-09-15

Primary input: [business-first career-shifter review](../../../../docs/maintainers/business-first-career-shifter-review.md)

### Objective

Make the shortest useful learner route read as:

> **business problem → product objective → build → engineering problem →
> concept → earned tool → done**

A career shifter should understand who needs the product, what useful behavior
they are adding, and why it matters before seeing terminology, packages, gate
codes, evidence mechanics, or maintainer status. Simplification must preserve
the M0–M10 capability arc, build/break/debug/repair practice, deterministic
proof, recovery, Core/optional boundaries, and the prohibition on supplied
finished learner solutions.

Air is the primary human-facing web layer throughout the target curriculum.
FastAPI remains the backend/API layer for JSON contracts, OpenAPI, dependencies,
health, and readiness. This is one application architecture, not a separate
frontend track: Air pages and FastAPI routes reuse the same use cases, services,
models, and database logic in one ASGI process.

### Authority and superseded presentation decisions

The 2026-09-15 maintainer decision explicitly reopens presentation and
information-architecture choices previously described as frozen or
non-negotiable. The following are no longer constraints:

1. the six-column technical roadmap in the root README;
2. exactly seven files in every milestone;
3. the fixed `Why → Starting checkpoint → Terms → Product brief` controller
   order;
4. a separate up-front `Terms used here` section;
5. repeated readiness disclaimers, five-cue legends, gate codes, resume
   warnings, and command-map tables in every controller;
6. technical capability names as the primary learner-facing milestone titles;
7. completed maintenance chronology remaining prominent in active owner files;
8. a FastAPI-only API being sufficient as the learner-visible product surface.

Earlier decisions and completed evidence remain historical records. They do not
justify preserving learner friction. `AGENTS.md`, validators, root navigation,
and authoring guidance must be migrated deliberately before lesson rollout so
the repository never has two active contracts.

The following constraints remain protected:

- stable labels M0–M10 and their capability progression;
- the five evolving product contexts and a minimal Air/FastAPI modular monolith;
- Air as the primary HTML/page/form/fragment layer and FastAPI as the retained
  JSON/OpenAPI and operational layer;
- one fixed Core route with Stretch and provider work clearly separate;
- executable tests, deliberate failures, debugging, recovery, evidence, and
  safe resume points;
- honest local, hosted, provider, deployment, and named-human evidence scopes;
- no completed learner solution, fabricated result, or hidden weakening of an
  acceptance gate.

### Target learner architecture

#### Root entrance

The root README should contain only what a new learner needs to begin:

1. who the course is for and what useful products they will evolve;
2. one setup path with expected result and recovery;
3. one four-column route map:
   `Milestone | Business problem | Product capability | Start here`;
4. a short explanation of the build/break/debug/repair loop;
5. secondary links to detailed curriculum, stack, quality, and maintainer
   references.

Concept inventories belong in `CURRICULUM.md`; tool policy belongs in `STACK.md`;
learner state belongs in `PROGRESS.md`; maintenance status belongs under
`docs/maintainers/`. None should compete with the first action.

#### Air/FastAPI application boundary

Each project should run as one application, process, port, and deployment unit:

```text
browser → Air pages/forms/HTMX fragments ─┐
                                          ├→ shared use case/service → database
client  → FastAPI JSON/OpenAPI routes ────┘
```

The composition root creates the existing FastAPI application, wraps it with
`air.Air(fastapi_app=api)`, and includes an `AirRouter` excluded from OpenAPI.
Keep existing JSON and operational paths stable. Put human-facing routes at `/`
and `/app/...`. Page and API handlers call shared Python behavior directly;
neither calls the other over HTTP. Do not mount disconnected applications whose
database pool, authentication, or state can drift.

Target current Air `0.48.1` exactly while it remains alpha. Its current runtime
requires a coordinated move from Python 3.12 to Python 3.13 across all five
`.python-version` files, project metadata, type-checker settings, lockfiles, CI,
documentation, and the POS container. Do not retain an older Air release merely
to avoid that explicit migration. Complete a Catalog preflight before changing
the other four starters.

Use Air Tags and small local components first. Use ordinary validated forms
before HTMX. Use explicit `AirForm.from_request()` rather than the currently
unstable dependency-injected form path. First earn HTMX from the M6 uncertain-
payment status need; first earn Air `SSEResponse` with the HTMX SSE extension in
M9. Jinja, AirDB, AirDragon/Tailwind, custom JavaScript, a SPA framework, Node,
and a separate frontend service remain excluded until evidence defeats the
simpler pattern.

#### Four-file milestone contract

Each milestone should converge on four learner-facing files:

| File | Single responsibility |
|---|---|
| `README.md` | Business problem, product objective, Air-visible behavior, starting observation, ordered build, just-in-time understanding, completion, and next action. |
| `CHALLENGE.md` | Deliberate failures, diagnostic prompts, progressive hints, reset, and recovery. |
| `ACCEPTANCE.md` | Core/Stretch boundary, observable proof, evidence destination, review questions, and completion claim. |
| `REFERENCE.md` | Concepts, Air/FastAPI patterns, tools, and sources organized as `Use now`, `Evaluate after evidence`, and `Do not add yet`. |

Merge `CONCEPTS.md`, `TOOLS.md`, and `RESOURCES.md` into `REFERENCE.md`. Merge
`REVIEW.md` into `ACCEPTANCE.md`. Preserve provenance and working links during
the migration; remove superseded files only after every inbound link and unique
content item is accounted for. This is consolidation, not silent deletion.

#### Milestone controller grammar

Every milestone README should use this learner-visible order:

1. **Business problem:** actor, current failure, and practical harm in no more
   than three short paragraphs.
2. **Product objective:** one user/operator-visible behavior and its explicit
   boundary. Prefer an Air page or form backed by the same behavior as the API.
3. **Start here:** last-green state, working directory, one command, expected
   observation, and recovery.
4. **Build:** three to six bounded blocks, each producing one useful behavior.
5. **Understand:** name and explain the engineering concept after the learner
   has observed the problem that creates its need.
6. **Use a tool if earned:** introduce a package or service only after stating
   the observed need, simpler alternative, operational cost, and removal
   condition.
7. **Prove it:** acceptance behavior and evidence destination without a
   duplicate command table.
8. **Done / next:** separate local Core completion from optional endorsements,
   then give one continuation link.

Air itself is established as the default UI at M0; its advanced features are
still earned progressively. A milestone must not introduce HTMX, SSE, a
component library, or another presentation abstraction before the relevant
browser problem is observable.

Use one compact block card: `Goal`, `Do`, `Expected`, `If not`, `Understand`,
and `Save/next`. A fact or command should have one canonical location. Supporting
links should point to the exact section needed at that moment.

#### Outcome-first naming and summaries

Keep stable milestone IDs and directory paths, but make primary visible titles
describe product progress:

| ID | Learner-facing title |
|---|---|
...
Replace artifact-first `You will leave with` summaries with:

- **Product can:** the useful behavior now available;
- **You will prove:** the smallest tests and operational evidence supporting it.

Technical capability names remain secondary metadata for traceability.

### Implementation sequence

#### BF0 — Freeze the replacement contract and Air boundary

Update the repository working contract and canonical authoring guidance to name
the four-file structure, business-first controller grammar, outcome-first root
map, just-in-time terminology, Air-primary/FastAPI-backend boundary, Python 3.13
target, exact Air pin, and evidence boundaries. Add transition rules so a
partially migrated tree is recognized as work in progress rather than falsely
passing either contract. The current Python 3.12 starters remain truthful until
the Catalog preflight changes code and lockfiles.

Exit: one active presentation contract exists, its protected constraints are
explicit, Air and FastAPI have non-overlapping primary responsibilities, and
`TODO.md` is the only active execution ledger.

#### BF1 — Build the Catalog M0–M1 Air and learner-path pilot

Preflight Python 3.13 and exactly pinned Air on the Catalog only. Preserve the
existing FastAPI `/health`, `/products/sample`, JSON response models, M1 contract
routes, and OpenAPI while making `air.Air` the exported ASGI app. Register an
OpenAPI-excluded Air router, render one M0 catalog page with Air Tags, and add one
ordinary Pydantic-backed Air form in M1. Consolidate the M0/M1 learner files to
the four-file contract and teach Air only after the visible product need.

Exit: one process serves Air HTML and unchanged FastAPI API behavior; HTML,
invalid-form, API-regression, OpenAPI, lint, type, and startup checks pass; and a
local fresh-context reader can identify the product, first action, proof, and
next step without opening a reference file.

#### BF2 — Stress-test the pattern on M2, M6, and M9

Apply the accepted composition and four-file pattern to M2 because it exposes
the clearest block/command duplication and must make stock persistence visible
without duplicating database logic. Apply it to M6 to earn the first HTMX
fragment from uncertain payment status. Apply it to M9 to prove that query
measurement, Redis, and Air SSE can be taught just in time without weakening
performance or reconnect/loss semantics. M9 owns the first earned SSE
interaction.

Exit: the pattern works for the entrance, a persistence lesson, and an advanced
interaction/performance lesson. Record `ROLL OUT`, `REVISE`, or `ROLL BACK`
before broad migration.

#### BF3 — Roll out through the remaining milestones

Migrate M3–M5, M7–M8, and M10 in product order, plus the remaining project shells. Add
one thin Air surface when POS, ecommerce, booking, and social first appear;
reuse their existing FastAPI routes, domain seams, SQLAlchemy/Alembic setup,
failure harnesses, and database logic. Preserve stable C/A IDs, commands, tests,
evidence paths, recovery, and Core/Stretch boundaries. Give each milestone a
three-sentence business mini-brief and `Product can` / `You will prove` summary
derived from its existing product contract.

Exit: all eleven controllers follow one causal learner grammar, every product
has a bounded Air UI over the same backend behavior, and no required API,
business invariant, failure exercise, or proof has disappeared.

#### BF4 — Simplify the course entrance and supporting references

Replace the root six-column technical table with the four-column business route
map. Move detail, rather than duplicate it, into `CURRICULUM.md`, `STACK.md`, and
the milestone `REFERENCE.md` files. Explain Air as the human-facing layer and
FastAPI as the retained backend/API only after the product route is clear. Keep
maintenance navigation clearly outside the learner path.

Exit: the first screen explains the course's products and next action before
tools or process, and every route-map row lands on the correct controller.

#### BF5 — Consolidate files and maintenance history safely

After links and unique-content inventories pass, remove the superseded milestone
files in a reviewed batch. Move completed planning chronology to a dated
maintainer archive while retaining short canonical `PLAN.md`, `TODO.md`, and
`USABILITY.md` front doors or indexes. Do not archive active external evidence
items or erase implementation history.

Exit: learner-visible file choice and active maintainer navigation are both
materially smaller, with an explicit provenance/rollback manifest.

#### BF6 — Validate, audit, repair, and close locally

Migrate validators and mutation tests to the four-file contract, stable
business-first facts, Python 3.13 consistency, exact Air pin, Air/FastAPI route
separation, OpenAPI preservation, and absence of a second frontend runtime. Run
the full local curriculum and starter verification matrix, audit every
removed/merged source, perform fresh-context local traversal, repair defects,
and update the TODO after each verified slice.

Exit: local automation passes; no broken links, missing unique content, gate
regression, solution leakage, or false evidence claim remains. Hosted rendering,
provider experiments, real deployment, and named-human study remain separately
unchecked until explicitly authorized and evidenced.

### Acceptance criteria

The simplification is locally complete only when:

1. a fresh-context reader can find the actor, harm, product objective, first
   action, expected observation, recovery, and done condition from each
   controller without reading supporting references first;
2. every milestone has exactly the four target files and every removed file has
   a recorded merge destination;
3. no controller contains a duplicate command map, repeated repository-status
   disclaimer, five-cue legend, or front-loaded term inventory;
4. each Core block retains a working directory, literal command or explicit
   create-then-run target, observable result, recovery, evidence, and resume
   point;
5. every tool is classified `Use now`, `Evaluate after evidence`, or `Do not add
   yet`, and required tools appear only after their need is explained;
6. M10 states independent local-Core and optional real-target completion
   conditions;
7. all stable C/A traces, learner-owned work, tests, failure/recovery exercises,
   and evidence boundaries remain intact;
8. curriculum validator, controlled mutations, relative-link/anchor checks,
   affected starter checks, and `git diff --check` pass;
9. all provider, hosted, deployment, and named-human work remains unchecked
   unless its own authorized evidence exists;
10. all five starters consistently use Python 3.13 and the exact reviewed Air
    pin, with lockfiles and the POS image regenerated;
11. Air owns HTML pages/forms/fragments, FastAPI retains JSON/OpenAPI and
    operational routes, and both reuse shared Python behavior in one process;
12. existing API contracts remain green, page routes do not pollute OpenAPI,
    form errors preserve safe input, and HTMX/SSE appear only at their earned
    M6/M9 points.

### Explicit non-goals

- Do not add a website, learning-management system, dashboard, or second track.
- Do not add React or another SPA framework, Node tooling, a separate frontend
  service, AirDB, Jinja, AirDragon/Tailwind, or browser-to-own-API HTTP calls as
  part of the baseline migration.
- Do not weaken acceptance gates, remove debugging practice, or replace evidence
  with prose confidence.
- Do not rewrite all milestones before the M0/M1 Catalog plus M2/M6/M9 pilot
  decision.
- Do not commit, push, publish, deploy, contact providers, or recruit human
  reviewers without separate authorization.
