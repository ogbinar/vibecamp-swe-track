...
- **A5:** No accepted durable obligation relies on FastAPI `BackgroundTasks`; documentation makes the kill boundary and at-least-once—not exactly-once—claim explicit. If Taskiq is retained, evidence names the broker/acknowledgement/redelivery/result/shutdown policies and repeats C1–C3 against that path.
